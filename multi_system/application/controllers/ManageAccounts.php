		...
		public function edit_record($id)
			...
			*/
			...
			$response =	$this->common_model->update($data,'manage_account',array('id'=>$id));
				/* if get any response display message*/
				if($response > 0 )
					{
						//display success msg
			...
			public function myaccountSetting()
			{
				...
				else
				{
					...
					if($result>0)
					{
						...
						$this->session->set_flashdata('success_message', 'Your Record is Edit Successfully...');
					
						}
					else{	$this->session->set_flashdata('error_message', '333');}		
